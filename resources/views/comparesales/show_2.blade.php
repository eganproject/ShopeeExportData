                 ...
                 <div class="card custom-card animated-card">
                     <div class="card-body">
                         <div class="d-flex justify-content-center align-items-center flex-wrap gap-4">
                             ...
                             <div class="col-md-4">
                                 <label for="channel" class="form-label"><i class="fas fa-broadcast-tower me-2"></i>Channel</label>
        ...
        <div class="row g-4 mb-4">
            <div class="col-lg-8">
                <div class="card custom-card h-100 animated-card" style="animation-delay: 0.4s;">
                    <div class="card-body">
                        <h5 class="fw-bold text-center"><i class="fas fa-chart-bar card-title-icon"></i>Grafik Penjualan (Bulan Ini)</h5>
            ...
            <div class="col-lg-4">
                <div class="card custom-card h-100 animated-card" style="animation-delay: 0.5s;">
                    <div class="card-body">
                        <h5 class="fw-semibold text-center"><i class="fas fa-pie-chart card-title-icon"></i>Distribusi Platform (Bulan Ini)</h5>
        ...
        <div class="row g-4 mb-4">
            <div class="col-lg-8">
                <div class="card custom-card h-100 animated-card" style="animation-delay: 0.6s;">
                    <div class="card-body">
                        <h5 class="fw-semibold text-center"><i class="fas fa-star card-title-icon"></i>Top 10 SKU (Bulan Ini vs Bulan Lalu)</h5>
                        <p class="fw-bold mb-3 text-muted text-center">Berdasarkan Total Pendapatan</p>
                        <div class="chart-container">
            ...
            <div class="col-lg-4">
                <div class="card custom-card h-100 animated-card" style="animation-delay: 0.7s;">
                    <div class="card-body">
                        <h5 class="fw-semibold text-center"><i class="fas fa-balance-scale card-title-icon"></i>Perbandingan Pendapatan Bulanan</h5>
        ...
        <div class="row g-4 mb-4">
            <div class="col-12">
                <div class="card custom-card h-100 animated-card" style="animation-delay: 0.8s;">
                    <div class="card-body">
                        <h5 class="fw-semibold text-center"><i class="fas fa-percentage card-title-icon"></i>Pertumbuhan Pendapatan per Periode</h5>
        ...
        <div class="row g-4 mb-4">
            <div class="col-12">
                <div class="card custom-card animated-card" style="animation-delay: 0.9s;">
                    <div class="card-body">
                        <h2 class="fw-semibold mb-3 text-center"><i class="fas fa-tags card-title-icon"></i>Sub Kategori</h2>
        ...
        <div class="row g-4 mb-4">
            <div class="col-12">
                <div class="card custom-card animated-card" style="animation-delay: 1.0s;">
                    <div class="card-body">
                        <h5 class="fw-semibold mb-3 text-center"><i class="fas fa-list-alt card-title-icon"></i>Detail Pendapatan per SKU</h5>
    ...
    <script>
        ...
        function updateKPIsAndCharts(data) {
            ...
            const processedData = data.map(item => {
                const p = {}, prev_p = {};
                for (let i = 1; i <= 4; i++) {
                    p[i] = parseFloat(channel === 'semua' ? item[`pendapatan_per_${i}`] : (channel === 'shopee' ? item[`pendapatan_shopee_per_${i}`] : item[`pendapatan_tiktok_per_${i}`])) || 0;
                    prev_p[i] = parseFloat(channel === 'semua' ? item[`prev_pendapatan_per_${i}`] : (channel === 'shopee' ? item[`prev_pendapatan_shopee_per_${i}`] : item[`prev_pendapatan_tiktok_per_${i}`])) || 0;
                }
                ...
                return { ...item, itemCurrentMonthTotal, itemPrevMonthTotal };
            });

            $('#kpi-total-revenue').text(formatRupiah(totalCurrentMonthRevenue));

            const growth = (totalPrevMonthRevenue > 0) ? ((totalCurrentMonthRevenue - totalPrevMonthRevenue) / totalPrevMonthRevenue) * 100 : (totalCurrentMonthRevenue > 0 ? 100 : 0);
            $('#kpi-growth').text(`${growth.toFixed(2)}%`).removeClass('text-success text-danger').addClass(growth >= 0 ? 'text-white' : 'text-warning');
            ...
            processedData.forEach(item => {
                 for (let i = 1; i <= 4; i++) {
                     let periodValue = 0;
                      ...
                      else if (channel === 'shopee') periodValue = parseFloat(item[`pendapatan_shopee_per_${i}`]) || 0;
            ...
            window.barTop10 = new Chart(document.getElementById('barTop10'), {
                ...
                options: { responsive: true, maintainAspectRatio: false, indexAxis: 'y', scales: { y: { beginAtZero: true } } }
            });
            ...
            for (let i = 1; i < currentMonthPeriodTotals.length; i++) {
                ...
                const growthPercentage = prevPeriod > 0 ? ((currentPeriod - prevPeriod) / prevPeriod) * 100 : (currentPeriod > 0 ? 100 : 0);
            ...
            window.periodGrowthChart = new Chart(document.getElementById('periodGrowthChart'), {
                ...
                data: {
                    ...
                    datasets: [{
                        ...
                        data: periodGrowth,
                        backgroundColor: periodGrowth.map(g => g >= 0 ? 'rgba(25, 135, 84, 0.8)' : 'rgba(220, 53, 69, 0.8)'),
                    }]
                },
                options: {
                    responsive: true,
        ...
        function putTable(data, tableId) {
            ...
            data.forEach((item, index) => {
                ...
                let d = {
                    1: p[1] - prev_p[4], 2: p[1] - prev_p[1], 3: p[2] - p[1],
                    4: p[2] - prev_p[2], 5: p[3] - p[2], 6: p[3] - prev_p[3],
                    7: p[4] - p[3],     8: p[4] - prev_p[4]
                };
                let pct = {
            ...
            const total_d = {
                1: totals.p[1] - totals.prev_p[4], 2: totals.p[1] - totals.prev_p[1], 3: totals.p[2] - totals.p[1],
                4: totals.p[2] - totals.prev_p[2], 5: totals.p[3] - totals.p[2], 6: totals.p[3] - totals.prev_p[3],
                7: totals.p[4] - totals.p[3],     8: totals.p[4] - totals.prev_p[4]
            };
            const total_pct = {
            ...
            let tfoot = `<tr class="fw-bold bg-light"><td colspan="${tableId === 'kategori-table' ? 3 : 2}" class="text-end">Total</td>`;
            tfoot += `<td class="text-end">${formatRupiah(totals.prev_p[4])}</td><td class="text-end">${formatRupiah(totals.p[1])}</td><td class="text-center">${renderSelisih(total_d[1], total_pct[1])}</td>`;
            tfoot += `<td class="text-end">${formatRupiah(totals.prev_p[1])}</td><td class="text-end">${formatRupiah(totals.p[1])}</td><td class="text-center">${renderSelisih(total_d[2], total_pct[2])}</td>`;
            tfoot += `<td class="text-end">${formatRupiah(totals.p[1])}</td><td class="text-end">${formatRupiah(totals.p[2])}</td><td class="text-center">${renderSelisih(total_d[3], total_pct[3])}</td>`;
            tfoot += `<td class="text-end">${formatRupiah(totals.prev_p[2])}</td><td class="text-end">${formatRupiah(totals.p[2])}</td><td class="text-center">${renderSelisih(total_d[4], total_pct[4])}</td>`;
            tfoot += `<td class="text-end">${formatRupiah(totals.p[2])}</td><td class="text-end">${formatRupiah(totals.p[3])}</td><td class="text-center">${renderSelisih(total_d[5], total_pct[5])}</td>`;
            tfoot += `<td class="text-end">${formatRupiah(totals.prev_p[3])}</td><td class="text-end">${formatRupiah(totals.p[3])}</td><td class="text-center">${renderSelisih(total_d[6], total_pct[6])}</td>`;
            tfoot += `<td class="text-end">${formatRupiah(totals.p[3])}</td><td class="text-end">${formatRupiah(totals.p[4])}</td><td class="text-center">${renderSelisih(total_d[7], total_pct[7])}</td>`;
            tfoot += `<td class="text-end">${formatRupiah(totals.prev_p[4])}</td><td class="text-end">${formatRupiah(totals.p[4])}</td><td class="text-center">${renderSelisih(total_d[8], total_pct[8])}</td>`;
            tfoot += '</tr>';
